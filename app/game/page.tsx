...
const Dashboard = () => {
    ...
    const [showCreateCombatControls, setShowCreateCombatControls] = useState(false);
    ...
    return (
        ...
        {isLoggedIn && clickedButton === "combat" && (
            <div className="popup visible">
            <div className="popup-content">
            <div className="combat-session-buttons">
            <button className="start-combat-btn" onClick={handleJoinCombatSession}>
            Join Combat Session
            </button>
            <button
            className="start-combat-btn"
            onClick={() => setShowCreateCombatControls(true)}
            >
            Create Combat Session
            </button>
            </div>
            
            {showCreateCombatControls && (
                <>
                ...
                <div className="popup-header">⏱ Duration:</div>
                <select className="time-select" value={combatTime} onChange={handleCombatTimeChange}>
                {Array.from({ length: 12 }, (_, i) => (i + 1) * 5).map((minutes) => (
                    <option key={minutes} value={minutes}>
                    {minutes} minutes
                    </option>
                ...
                {combatMode === "team" && (
                    ...
                    <select className="time-select" value={teamSize} onChange={handleTeamSizeChange}>
        ...
        {/* GUEST USERS SEE ONLY JOIN OPTION */}
        {!isLoggedIn && clickedButton === "combat" && (
            <div className="popup visible">
            <div className="popup-content">
            <button className="start-combat-btn" onClick={handleJoinCombatSession}>
